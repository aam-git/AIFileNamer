<script lang="ts">
  ...
  let lastNonSettingsView: 'scanners' | 'scanner-detail' | 'about' | 'global-settings' = 'scanners';
  ...
  function toggleSettings() {
    if (topView === 'global-settings') {
      topView = lastNonSettingsView === 'global-settings' ? 'scanners' : lastNonSettingsView;
    } else {
      lastNonSettingsView = topView;
      topView = 'global-settings';
    }
  }
  ...
  onMount(async () => {
    ...
    if (window.electronAPI?.onTrayAction) {
      cleanupTray = window.electronAPI.onTrayAction((action: string) => {
    ...
    if (window.electronAPI?.onUpdateReady) {
      cleanupUpdate = window.electronAPI.onUpdateReady(() => {
        updateReady = true;
      });
    }
  });
...
<div class="flex flex-col h-screen w-full bg-slate-950 overflow-hidden font-sans">
  ...
  <div class="h-9 w-full bg-[#0f172a] border-b border-slate-800/80 flex items-center justify-between px-3 select-none z-50 shrink-0" style="-webkit-app-region: drag;">
    ...
    <div class="flex items-center gap-1" style="-webkit-app-region: no-drag;">
      ...
      <button onclick={toggleSettings} aria-label="Global Settings" class="w-7 h-7 rounded flex items-center justify-center hover:bg-slate-700 transition-colors outline-none mr-2 {topView === 'global-settings' ? 'text-indigo-400 bg-slate-800' : 'text-slate-400 hover:text-white'}">
  ...
  {#if updateReady}
    <div class="fixed bottom-4 right-4 z-[9999] bg-slate-800 border border-indigo-700 text-white px-4 py-3 rounded-lg shadow-2xl flex items-center gap-3 animate-fade-in select-none">
      <div class="flex flex-col">
        <span class="text-sm font-semibold text-indigo-400">Update Ready</span>
        <span class="text-xs text-slate-300">A new version has been downloaded.</span>
      </div>
      <button class="bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold px-3 py-1.5 rounded transition-colors ml-2 cursor-pointer shadow-lg shadow-indigo-900/50" onclick={() => window.electronAPI.installUpdate()}>
        Restart to Install
      </button>
      <button class="p-1 hover:bg-slate-700 text-slate-400 hover:text-white rounded transition-colors ml-1" onclick={() => updateReady = false} title="Dismiss">
        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
      </button>
    </div>
  {/if}
